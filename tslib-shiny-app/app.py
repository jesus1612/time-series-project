...
def server(input, output, session):
    ...
    def forecast_diff_horizon_plot():
        ...
        state = app_state.get()
        if not state.get("analysis_complete") or state.get("model_type") not in (
            "ARIMA",
            "AR",
            "MA",
        ):
    ...
    def handle_start_execution():
        ...
        try:
            ...
            if model_type == "ARIMA":
                ...
                new_state = app_state.get().copy()
                new_state["execution_log"].append(
                    "Ajustando MA paralelo (Spark, ParallelMAWorkflow)..."
                )
                app_state.set(new_state)
                try:
                    t_par0 = time.perf_counter()
                    ...
                    new_state = app_state.get().copy()
                    backend = getattr(parallel_workflow, "backend_", "desconocido")
                    new_state["execution_log"].append(f"✓ MA paralelo completado (backend: {backend})")
                    app_state.set(new_state)
                except Exception as e:
                    error_msg = f"Ruta paralela Spark (MA): {type(e).__name__}: {str(e)}"
                    logger.error(error_msg)
                    logger.error(traceback.format_exc())
                    new_state = app_state.get().copy()
                    new_state["execution_log"].append(f"⚠ {error_msg}")
                ...
                if parallel_workflow is not None and getattr(parallel_workflow, "order_", None) is not None:
                    qo = parallel_workflow.order_
                    sm_order = (0, 0, int(qo[0]))
                    new_state = app_state.get().copy()
                    new_state["execution_log"].append(
                        f"Orden statsmodels alineado: ARIMA{sm_order} (MA clásico sobre serie de trabajo)"
                    )
                    app_state.set(new_state)
                elif order is not None and len(order) >= 1:
                    sm_order = (0, 0, int(order[0]))
                else:
                    sm_order = (0, 0, 1)
                    runtime_msgs.append(
                        "MA lineal: orden (0,0,1) por defecto (sin orden del workflow ni q manual)."
                    )

                new_state = app_state.get().copy()
                new_state["execution_log"].append("Ajustando MA lineal (statsmodels, ARIMA(0,0,q))...")
                app_state.set(new_state)

                t_lin0 = time.perf_counter()

                def _fit_linear_ma_statsmodels():
                    if parallel_workflow is not None and getattr(
                        parallel_workflow, "working_data_", None
                    ) is not None:
                        return tslib_service.fit_statsmodels_ma_aligned_to_parallel_ma_workflow(
                            parallel_workflow
                        )
                    return tslib_service.fit_statsmodels_arima(
                        np.asarray(data, dtype=float), sm_order
