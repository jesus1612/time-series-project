"""
Parallel MA Workflow (classic MA(q) on stationary working data)
...
family is **MA(q)** only using ``MAProcess`` (MLE). Preprocessing (log + differencing)
matches Step 1 of ``ParallelARIMAWorkflow`` so the series passed to MA is fitted on stationary.
...
class ParallelMAWorkflow:
    """
    Full parallel workflow for classic MA(q) with Spark-accelerated grid search.

    Attributes
    ----------
    order_ : tuple
        Selected order ``(q,)`` (compatible with app / TSLib MA).
    working_data_ : ndarray
        Series used for the final MA fit (after log / differencing).
    differencing_order_ : int
        Number of differences applied in Step 1 (for documentation).
    parameters_ : dict
        phi (empty), theta, c (mean), sigma2 from the final ``MAProcess``.
    """
    ...
    def __init__(
        ...
        if spark_session is None:
            ...
            self.spark = ensure_spark_session(
                ...
                spark_config=cfg,
                master=master,
                app_name=app_name or "TSLib-ParallelMAWorkflow",
        ...
        self._final_model: Optional[MAProcess] = None
    ...
    def _apply_grid_mode(
        ...
        if self._grid_mode == "manual":
            if self._manual_max_q is None:
                raise ValueError("grid_mode='manual' requires manual_max_q.")
            out = {**config, "max_q": int(self._manual_max_q)}
            return out
        if self._grid_mode == "acf_pacf":
            cap = int(config["max_q"])
            _mp, mq, meta = suggest_p_q_orders(
                working_data,
                d_prep,
            ...
            return {**config, "max_q": mq}
        return config

    def _q_candidates(self, max_q: int) -> List[int]:
        return [q for q in range(1, max_q + 1)]
    ...
    def _fit_sliding_spark(self, windows: List[Dict[str, Any]], q_list: List[int]) -> pd.DataFrame:
        ...
        if not tasks_list:
            raise ValueError("Parallel MA: no sliding tasks.")
        ...
        def fit_map(iterator):
            ...
            try:
                from tslib.core.arima import MAProcess
            ...
            for pdf in iterator:
                for _, row in pdf.iterrows():
                    ...
                    try:
                        ...
                        mu = float(model.mean if hasattr(model, "mean") else 0.0)
        ...
        if self.verbose:
            print("  🚀 Parallel MA sliding fit (Spark mapInPandas)...")
            t0 = _time.time()
    ...
    def _select_global_q(self, results_df: pd.DataFrame) -> Tuple[int, float]:
        ok = results_df[results_df["success"] == True].copy()
        if len(ok) == 0:
            raise ValueError("No successful MA fits in sliding step.")
        ok["rank"] = ok.groupby("window_id")["aicc"].rank(method="min")
        ms = ok.groupby("q").agg({"rank": ["mean", "std", "count"], "aicc": ["mean", "std"]}).reset_index()
        ms.columns = ["q", "rank_mean", "rank_std", "count", "aicc_mean", "aicc_std"]
        ...
        return q_best, float(conf)

    def _reconcile_full_sample(
        self, working_data: np.ndarray, sliding_q: int, conf: float
    ) -> Tuple[int, float]:
        ...
        if ms is None or len(ms) == 0:
            ...
            return sliding_q, conf

        data = np.asarray(working_data, dtype=float).ravel()
        k = min(self._full_sample_reconcile_top_k, len(ms))
        top = ms.nsmallest(k, "score")
        candidates = list(dict.fromkeys([int(r["q"]) for _, r in top.iterrows()]))
        if sliding_q not in candidates:
            candidates.insert(0, sliding_q)

        aic_by_q: Dict[int, float] = {}
        for q in candidates:
            try:
                m = MAProcess(order=q, n_jobs=1)
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    m.fit(data)
                aic_by_q[q] = float(m._fitted_params["aic"])
            except Exception:
                continue
        if not aic_by_q:
            return sliding_q, conf
        best_q = min(aic_by_q.keys(), key=lambda x: aic_by_q[x])
    ...
    def _create_fixed_windows(
        ...
        n = len(data)
        q_ord = self.order_[0] if self.order_ else 1
        window_size = n // num_windows
        min_required = q_ord + 10
    ...
    def _backtest(self, windows: List[Dict[str, Any]], q: int) -> pd.DataFrame:
        if not windows:
            return pd.DataFrame()
        ...
        def bt_map(iterator):
            ...
            for pdf in iterator:
                for _, row in pdf.iterrows():
                    ...
                    try:
                        model = MAProcess(order=qq, n_jobs=1)
                        with warnings.catch_warnings():
                            warnings.simplefilter("ignore")
    ...
    def _diagnose(self, windows: List[Dict[str, Any]], q: int) -> pd.DataFrame:
    ...
    def _local_adjust(self, data: np.ndarray, q: int) -> int:
        try:
            base = MAProcess(order=q, n_jobs=1)
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                base.fit(data)
            bp = base._fitted_params
            n_obs = len(data)
            base_aicc = InformationCriteria.aicc(bp["log_likelihood"], q + 1, n_obs)
        except Exception:
            return q
        best_q, best_aicc = q, base_aicc
        for cand in (q + 1, q - 1):
            if cand < 1:
                continue
            try:
                m = MAProcess(order=cand, n_jobs=1)
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    m.fit(data)
                fp = m._fitted_params
                ...
                continue
        self.results_["step10_adjustment"] = {"from_q": q, "to_q": best_q}
        return best_q

    def fit(self, data: Union[np.ndarray, pd.Series, list]) -> "ParallelMAWorkflow":
        ...
        q_list = self._q_candidates(int(cfg["max_q"]))
        self.results_["step2_3_grid"] = {"max_q": cfg["max_q"], "q_list": q_list}

        sliding_windows = self._create_sliding_windows(
            working_data, cfg["num_sliding_windows"], cfg["overlap_sliding"]
        )
        slide_df = self._fit_sliding_spark(sliding_windows, q_list)
        best_q, conf = self._select_global_q(slide_df)
        if self._full_sample_reconcile:
            best_q, conf = self._reconcile_full_sample(working_data, best_q, conf)
        self.order_ = (best_q,)

        fixed_w = self._create_fixed_windows(working_data, cfg["num_fixed_windows"])
        self._backtest(fixed_w, best_q)
        diag_df = self._diagnose(fixed_w, best_q)
        need_adj, _ = self._check_failures(diag_df)
        if need_adj:
            best_q = self._local_adjust(working_data, best_q)
            self.order_ = (best_q,)

        q_final = self.order_[0]
        self._final_model = MAProcess(order=q_final, n_jobs=1)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
        ...
        self.parameters_ = {
            ...
            "c": float(self._final_model.mean if hasattr(self._final_model, "mean") else 0.0),
