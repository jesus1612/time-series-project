"""
MA model: sequential n_jobs, Spark, optional statsmodels ARIMA(0,0,q).
"""
...
@pytest.fixture
def ma_data():
    return ma_series_dict(n_series=5, n_obs=100, seed=202)


def test_ma_sequential_n_jobs_variants(ma_data):
    order = 2
    steps = 5
    _, t1 = timed_call(
        forecasts_from_sequential, ma_data, "MA", order, steps, n_jobs=1
    )
    fc2, t2 = timed_call(
        forecasts_from_sequential, ma_data, "MA", order, steps, n_jobs=-1
    )
    assert len(fc2) == len(ma_data)
    for sid in ma_data:
        assert np.all(np.isfinite(fc2[sid]))
    print(f"[MA sequential] n_jobs=1: {t1:.4f}s, n_jobs=-1: {t2:.4f}s")


@pytest.mark.spark
def test_ma_spark_vs_sequential(ma_data, spark_session_benchmark):
    ...
    rows = [(sid, float(v)) for sid, y in ma_data.items() for v in y]
    ...
    assert len(collected) == len(ma_data) * steps
    ...
    print(
        f"[MA] sequential: {t_seq:.4f}s, spark: {t_spark:.4f}s, "
        f"parallel vs sequential agreement: {agree:.2f}% (experimental Spark path)"
    )


def test_ma_statsmodels_reference_alignment(ma_data):
    ...
    m = MAModel(order=q, auto_select=False, validation=False, n_jobs=1)
    m.fit(y)
    fc_ts = m.predict(steps=steps)

    sm = ARIMA(y, order=(0, 0, q), trend="c").fit()
    fc_sm = np.asarray(sm.forecast(steps=steps), dtype=float)

    mad = mean_abs_diff(fc_ts, fc_sm)
    assert mad < max(0.6, 0.2 * np.std(y)), f"MA reference misaligned, MAD={mad}"
